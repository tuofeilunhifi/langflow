...
def set_oss_env(ak_id: str, ak_secret: str, hosts: Union[str, List[str]],
    ...
    ENVOSSIO.access_oss(ak_id, ak_secret, hosts, buckets)
...
class OSSIO(IOLocal):
    ...
    def _get_default_oss_config(self, oss_cfg_config=None):
        oss_config = get_oss_config(oss_cfg_config)
    ...
    def access_oss_byconfig(self, oss_cfg_config):
        ak_id, ak_secret, hosts, buckets = \
                    self._get_default_oss_config(oss_cfg_config)
        self.access_oss(ak_id, ak_secret, hosts, buckets)
    ...
    def is_accessed(self, object_name='io'):
        if getattr(self, 'oss_config') is not None:
            return True
        else:
            raise BaseException("ossio_check for %s failed, should call %s.access_oss(ak_id='xxxx', ak_secert='xxxx', ak_secert='xxxx', buckets='xxxx')"%(object_name, object_name))
            return False
